...
SELECT Nombre, Costo, YEAR(FechaInicio) AS Año FROM Proyectos
...
SELECT RazonSocial, Cuit, TelefonoFijo, IDTipoCliente FROM Clientes WHERE IDTipoCliente IN (1,3,5,6)
...
SELECT Nombre, Costo, FechaInicio, FechaFin, IDCliente FROM Proyectos WHERE IDCliente NOT IN (1,5,10)
...
SELECT RazonSocial, Cuit, Email, TelefonoFijo AS Tel, TelefonoMovil AS Tel  FROM Clientes WHERE Email IS NOT NULL OR TelefonoFijo IS NOT NULL OR TelefonoMovil IS NOT NULL

--25 Listado de razón social, cuit y mail. Si no dispone de mail debe aparecer el texto "Sin mail".
SELECT RazonSocial, Cuit, ISNULL(Email,'Sin mail') AS Email FROM Clientes

--26 Listado de razón social, cuit y una columna llamada Contacto con el mail, 
--si no posee mail, con el número de celular, si no posee número de celular con el número de teléfono,
--de lo contrario un texto que diga "Incontactable".
SELECT RazonSocial, Cuit, Coalesce(Email, TelefonoMovil, TelefonoFijo, 'Incontactable') AS Contacto FROM Clientes

--27 Listado de razón social, cuit y una columna llamada Contacto con el mail,
--si no posee mail, con el número de celular y si no posee número de celular con un texto que diga "Incontactable".
